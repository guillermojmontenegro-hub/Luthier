...
VAGUE_TERMS = ("helpful", "useful", "good", "best", "general", "flexible", "powerful", "cosas", "varios")
...
def evaluate_rules(skill: DiscoveredSkill, metrics: SkillMetrics, profile: EvaluationProfile) -> list[Finding]:
    ...
    if metrics.description_tokens_estimate > 80:
        findings.append(
            Finding(
                ...
                recommendation="Compress the opening description to a sharper scope and trigger signal.",
            )
        )

    if metrics.description_tokens_estimate < 6 or any(term in skill.description.lower() for term in VAGUE_TERMS):
        findings.append(
            Finding(
    ...
    if any(signal in skill.platform_signals for signal in ("powershell", "cmd.exe", "windows")) and profile.operating_system != "windows":
        findings.append(
            Finding(
                code="platform-specific-instructions",
                severity="medium",
                message="The skill contains platform-specific instructions that may not be portable.",
                evidence=skill.platform_signals,
                recommendation="Document alternatives per OS or narrow the supported platform explicitly.",
            )
        )
    ...
    if len(active_languages) > 1:
        findings.append(
            Finding(
                ...
                evidence=[f"{lang}={skill.language_mix[lang]}" for lang in sorted(skill.language_mix)],
                recommendation="Keep the operational instructions in one dominant language unless multilingual support is required.",
            )
        )
    ...
    if metrics.restriction_count >= max(6, metrics.imperative_steps * 2):
        findings.append(
            Finding(
                ...
                evidence=[f"restrictions={metrics.restriction_count}", f"imperatives={metrics.imperative_steps}"],
                recommendation="Reduce rigid wording to the cases where it materially improves behavior.",
            )
        )
    ...
    if len(forced_tool_lines) >= 2 or (forced_tool_lines and _has_rigid_sequence(skill)):
        findings.append(
            Finding(
                ...
                recommendation="Keep hard requirements only for cases where compatibility or correctness depends on them.",
            )
        )

    if max(metrics.imperative_steps, list_line_count) >= 8 and metrics.example_count <= 1 and metrics.restriction_count >= 3:
        findings.append(
            Finding(
                code="over-specified-workflow",
                severity="medium",
                message="The skill prescribes a detailed workflow that may be narrower than its stated purpose.",
                evidence=[
                ...
                recommendation="Separate required constraints from optional guidance and compress procedural detail.",
            )
        )
    ...
    if duplicate_lines:
        findings.append(
            Finding(
                ...
                recommendation="Keep each instruction once and remove repeated wording that increases context cost.",
            )
        )
    ...
    if metrics.total_tokens_estimate > 700 and metrics.instruction_density < 0.02:
        findings.append(
            Finding(
                ...
                recommendation="Remove narrative content and keep only operational constraints and examples.",
            )
        )
